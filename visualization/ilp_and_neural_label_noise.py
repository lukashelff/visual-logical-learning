...
def label_noise_plot(outpath, training_samples=1000, vis='Trains'):
    ...
    for c, rule in enumerate(rules):
        ...
        if data_t.empty:
            continue
        for model, n in product(models, noise):
            data_temp = data_t.loc[data['Models'] == model].loc[data['noise'] == n]
            hatch = mt[model] if use_materials else mt[n]
            sns.barplot(x=material_category_name, order=material_category, y='Validation acc', hue=colors_category_name,
