...
class Ilp_trainer():
            ...
            if complete_run and os.path.exists(csv) and not pd.read_csv(open(csv)).empty:
                ...
                if model == 'popper':
                    ...
                    # for c, input in enumerate(inputs):
                        # print('nn')
                        # out.append(self.popper_train(input))
                    with mp.Pool(5) as p:
    ...
    def popper_train(self, path, train_log=False):
        ...
        settings = Settings(popper_data, debug=True, show_stats=train_log, quiet=not train_log, timeout=1000)
        # settings = Settings(popper_data, functional_test=True)
