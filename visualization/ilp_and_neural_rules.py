...
def compare(neural_path, ilp_pth, vis='Trains', im_count=1000):
...
def vis_ilp_and_neural(neural_path, ilp_pth, vis='Trains', im_count=1000):
    ilp_stats_path = f'{ilp_pth}/stats'
    ...
    leg = fig.legend(
        white +
        # color_markers + white*(len(models)-len(im_count)+1) +
        handels,
        # ['Training samples:'] + im_count + ['']*(len(models)-len(im_count)) +
